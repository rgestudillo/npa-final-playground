...
router.post("/run", async (req, res) => {
  let { language = "cpp", code, userInput, syntax } = req.body;
  ...
  try {
    ...
    job = await Job({ language, filepath, userInput, syntax }).save();
    const jobId = job["_id"];
    addJobToQueue(jobId);
    console.log("ADDED");
...
router.post("/submit", verify, async (req, res) => {
  let { language = "cpp", code, userInput, syntax, problemId, userId } = req.body;
